...
public class BlogPlugin extends XWikiDefaultPlugin{
  ...
  protected BaseClass getBlogConfigClass(XWikiContext context) throws XWikiException {
    XWikiDocument doc;
    XWiki xwiki = context.getWiki();
    boolean needsUpdate = false;

    try {
        doc = xwiki.getDocument("Celements2.BlogConfigClass", context);
    } catch (Exception e) {
  ...
  private BaseClass getArticleClass(XWikiContext context) throws XWikiException {
    XWikiDocument doc;
    XWiki xwiki = context.getWiki();
    boolean needsUpdate = false;

    try {
        doc = xwiki.getDocument("XWiki.ArticleClass", context);
    } catch (Exception e) {
        doc = new XWikiDocument();
        doc.setSpace("XWiki");
  ...
  private BaseClass getNewsletterConfigClass(XWikiContext context) throws XWikiException {
    XWikiDocument doc;
    XWiki xwiki = context.getWiki();
    boolean needsUpdate = false;

    try {
        doc = xwiki.getDocument("Classes.NewsletterConfigClass", context);
    } catch (Exception e) {
        doc = new XWikiDocument();
