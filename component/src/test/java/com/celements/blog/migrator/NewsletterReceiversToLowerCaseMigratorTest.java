...
public class NewsletterReceiversToLowerCaseMigratorTest extends AbstractBridgedComponentTestCase {
  ...
  public void testMigrate_oneCorrectResult() throws XWikiException {
    ...
    expect(store.search((String) anyObject(), eq(0), eq(0), same(getContext()))).andReturn(
        resultList).once();
    replay(store, wiki);
  ...
  public void testMigrate_twoIncorrect() throws XWikiException {
    ...
    DocumentReference lowerRef = new DocumentReference(getContext().getDatabase(), "Regspc",
        "Docname");
    XWikiDocument lowerDoc = new XWikiDocument(lowerRef);
    expect(wiki.getDocument(eq(lowerRef), same(getContext()))).andReturn(lowerDoc).times(2);
    BaseObject lowerObj = new BaseObject();
    lowerObj.setXClassReference(objRef);
    lowerObj.setNumber(0);
    lowerObj.setIntValue("isactive", 1);
