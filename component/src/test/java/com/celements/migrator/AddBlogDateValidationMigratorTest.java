...
public class AddBlogDateValidationMigratorTest extends AbstractBridgedComponentTestCase {
  ...
  public void testMigrate() throws Exception {
    ...
    assertTrue(((DateClass) bClass.get("publishdate")).getValidationRegExp().length(
        ) > 0);
    assertTrue(((DateClass) bClass.get("archivedate")).getValidationRegExp().length(
        ) > 0);
    ...
    assertEquals(((DateClass) bClass.get("publishdate")).getDateFormat(), 
        "dd.MM.yyyy HH:mm");
    assertEquals(((DateClass) bClass.get("archivedate")).getDateFormat(), 
        "dd.MM.yyyy HH:mm");
  }
